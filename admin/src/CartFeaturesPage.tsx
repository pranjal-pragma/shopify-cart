...
const newOneTickSkuRule = (): OneTickSkuRule => ({
  id: itemId(),
  text: richText('Add this to your cart'),
  variant_id: '',
  variant_title: '',
  applicable_on: 'all',
  trigger_ids: [],
...
export const defaultFeatures: CartFeaturesConfiguration = {
  ...
  one_tick_sku_rules: [],
  one_tick_remove_with_parent: false,
  one_tick_match_parent_quantity: false,
...
function TierScopeField({configuration, previewMode, onChange}: {configuration: CartFeaturesConfiguration; previewMode: boolean; onChange: (change: Pick<CartFeaturesConfiguration, 'tiered_applicable_ids' | 'tiered_applicable_titles' | 'tiered_applicable_product_ids'>) => void}) {
  if (configuration.tiered_applicable_on === 'all') return null;
  const type = configuration.tiered_applicable_on === 'products' ? 'product' : 'collection';
  const remove = (position: number) => onChange({
    tiered_applicable_ids: configuration.tiered_applicable_ids.filter((_, index) => index !== position),
    tiered_applicable_titles: configuration.tiered_applicable_titles.filter((_, index) => index !== position),
    tiered_applicable_product_ids: configuration.tiered_applicable_product_ids.filter((_, index) => index !== position),
  });
  const select = async () => {
    const resources = await pickResources(type, configuration.tiered_applicable_ids);
    onChange({
      tiered_applicable_ids: resources.map((resource) => resource.id),
      tiered_applicable_titles: resources.map((resource) => resource.title || resource.displayName || resource.id),
      tiered_applicable_product_ids: resources.map((resource) => (
        type === 'product' ? [resource.id] : (resource.products || []).map((product) => product.id)
      )),
    });
  };
  return <div className="field"><span>{type === 'product' ? 'Selected products' : 'Selected collections'}</span><button className="button button--secondary resource-picker-button" type="button" disabled={previewMode || !window.shopify?.resourcePicker} onClick={select}><PackageSearch size={16} />Select {type === 'product' ? 'products' : 'collections'}</button><div className="resource-chips">{configuration.tiered_applicable_titles.map((title, index) => <span key={configuration.tiered_applicable_ids[index]}>{title}<button type="button" onClick={() => remove(index)} aria-label={`Remove ${title}`}><X size={13} /></button></span>)}</div><small>Only matching cart products contribute toward reward goals.</small></div>;
}
...
function OneTickSkuRulesEditor({rules, previewMode, onChange}: {rules: OneTickSkuRule[]; previewMode: boolean; onChange: (rules: OneTickSkuRule[]) => void}) {
  const setRule = (index: number, next: OneTickSkuRule) => onChange(
    rules.map((rule, ruleIndex) => ruleIndex === index ? next : rule),
  );
  return <><div className="feature-item-list gift-offer-list">{rules.map((rule, index) => {
    const resourceType = rule.applicable_on === 'collections' ? 'collection' : 'product';
    const removeTrigger = (triggerIndex: number) => setRule(index, {
      ...rule,
      trigger_ids: rule.trigger_ids.filter((_, itemIndex) => itemIndex !== triggerIndex),
      trigger_titles: rule.trigger_titles.filter((_, itemIndex) => itemIndex !== triggerIndex),
      trigger_product_ids: rule.trigger_product_ids.filter((_, itemIndex) => itemIndex !== triggerIndex),
    });
    const selectTriggers = async () => {
      const resources = await pickResources(resourceType, rule.trigger_ids);
      setRule(index, {
        ...rule,
        trigger_ids: resources.map((resource) => resource.id),
        trigger_titles: resources.map((resource) => resource.title || resource.displayName || resource.id),
        trigger_product_ids: resources.map((resource) => (
          resourceType === 'product' ? [resource.id] : (resource.products || []).map((product) => product.id)
        )),
      });
    };
    return <details className="feature-item gift-offer" open key={rule.id}><summary><span><ChevronDown size={16} /><strong>Rule {index + 1}</strong></span><button className="button button--danger button--compact" type="button" onClick={(event) => { event.preventDefault(); onChange(rules.filter((item) => item.id !== rule.id)); }}>Remove rule</button></summary><div className="gift-offer__body"><RichField label="Text to display" value={rule.text} onChange={(value) => setRule(index, {...rule, text: value})} /><ResourceField label="Upsell product variant" type="variant" previewMode={previewMode} value={rule.variant_id || null} title={rule.variant_title} onChange={(resource) => setRule(index, {...rule, variant_id: resource?.id || '', variant_title: resource?.displayName || resource?.title || ''})} /><div className="field"><span>Applicable on</span><ChoiceGroup name={`One-tick rule ${index + 1} scope`} value={rule.applicable_on} onChange={(value) => setRule(index, {...rule, applicable_on: value as OneTickSkuRule['applicable_on'], trigger_ids: [], trigger_titles: [], trigger_product_ids: []})} options={[{value: 'all', label: 'All products'}, {value: 'products', label: 'Specific products'}, {value: 'collections', label: 'Specific collections'}]} /></div>{rule.applicable_on !== 'all' && <div className="field"><span>{rule.applicable_on === 'products' ? 'Selected products' : 'Selected collections'}</span><button className="button button--secondary resource-picker-button" type="button" disabled={previewMode || !window.shopify?.resourcePicker} onClick={selectTriggers}><PackageSearch size={16} />Select {rule.applicable_on}</button><div className="resource-chips">{rule.trigger_titles.map((title, triggerIndex) => <span key={rule.trigger_ids[triggerIndex]}>{title}<button type="button" onClick={() => removeTrigger(triggerIndex)} aria-label={`Remove ${title}`}><X size={13} /></button></span>)}</div></div>}</div></details>;
  })}</div><button className="text-button" type="button" disabled={rules.length >= 20} onClick={() => onChange([...rules, newOneTickSkuRule()])}><Plus size={16} />Add SKU/Product rule</button></>;
}
...
function RewardFulfillmentFields({configuration, discounts, reward, onChange}: {configuration: CartFeaturesConfiguration; discounts: ShopifyDiscountOption[]; reward: TierReward; onChange: (reward: TierReward) => void}) {
...
function validate(configuration: CartFeaturesConfiguration): string | null {
  ...
  if (configuration.tiered_rewards_enabled && configuration.tiered_applicable_on !== 'all' && !configuration.tiered_applicable_ids.length) return `Select at least one ${configuration.tiered_applicable_on === 'products' ? 'product' : 'collection'} for tiered rewards.`;
  ...
  if (configuration.tiered_rewards_enabled && configuration.tiered_rewards.some((reward) => ['shipping', 'discount'].includes(reward.reward_type) && !reward.discount_id)) return 'Select a Shopify discount for every shipping and discount reward.';
...
export function CartFeaturesPage({previewMode}: {previewMode: boolean}) {
  ...
  const selectView = (view: FeatureView) => {
    ...
    if (dirty) {
      setError('Save or discard your current changes before switching tabs.');
      return;
    }
    setError(null);
    setActiveView(view);
