...
5. The Shopify service publishes the same configuration to the app-owned cart metafield.
6. The app embed reads that metafield and applies the settings to Shopify Ajax Cart APIs.
7. For eligible free gifts, the theme adds a marked cart line and the Discount Function enforces
   the zero price at checkout.

## Storage and external systems

- PostgreSQL stores encrypted shop sessions, webhook deliveries, and cart configuration.
- Shopify Admin GraphQL handles app installation data, metafields, products, and discounts.
